...
class ClientAdminDataProvider {
  ...
  async createHousingProject(
    project: Omit<HousingProject, "id" | "created_at" | "updated_at">
  ) {
    const { data, error } = await this.supabase
      .from("housing_projects")
      .insert([project])
  ...
  async getLandPlots(filters?: {
    status?: string;
    featured?: boolean;
    limit?: number;
  }) {
    let query = this.supabase.from("land_plots").select("*");
  ...
  async createLandPlot(
    plot: Omit<LandPlot, "id" | "created_at" | "updated_at">
  ) {
    const { data, error } = await this.supabase
      .from("land_plots")
      .insert([plot])
  ...
  async getDownloads(filters?: {
    ...
    let query = this.supabase.from("downloads").select("*");
  ...
  async createDownload(
    download: Omit<Download, "id" | "created_at" | "updated_at">
  ) {
    const { data, error } = await this.supabase
      .from("downloads")
      .insert([download])
  ...
  async deleteDownload(id: number) {
    const { error } = await this.supabase.from("downloads").delete().eq("id", id);
    if (error) throw error;
  }

  async incrementDownloadCount(id: number) {
    const { data, error } = await this.supabase.rpc('increment_download_count', { download_id: id });
    if (error) throw error;
    return data;
  }
  ...
  async getContactInquiries(filters?: {
    ...
    let query = this.supabase.from("contact_inquiries").select("*");
  ...
  async createContactInquiry(
    inquiry: Omit<ContactInquiry, "id" | "created_at" | "updated_at">
  ) {
    const { data, error } = await this.supabase
      .from("contact_inquiries")
      .insert([inquiry])
  ...
  async getDashboardStats() {
    const [newsCount, housingCount, landCount, projectCount, inquiryCount] = await Promise.all([
      this.supabase.from("news").select("id", { count: "exact" }),
      this.supabase.from("housing_projects").select("id", { count: "exact" }),
      this.supabase.from("land_plots").select("id", { count: "exact" }),
      this.supabase.from("projects").select("id", { count: "exact" }),
      this.supabase.from("contact_inquiries").select("id", { count: "exact" })
    ]);

    return {
      news: newsCount.count || 0,
      housing: housingCount.count || 0,
      land: landCount.count || 0,
...
export default ClientAdminDataProvider;