...
class AdminDataProvider {
  ...
  async getDashboardStats() {
    const [newsCount, housingCount, landCount, projectCount, inquiryCount] =
      await Promise.all([
        this.supabase.from("news").select("*", { count: "exact", head: true }),
        this.supabase
          .from("housing_projects")
    ...
    const recentInquiries = await this.getContactInquiries({
      status: "new",
      limit: 5,
    });
